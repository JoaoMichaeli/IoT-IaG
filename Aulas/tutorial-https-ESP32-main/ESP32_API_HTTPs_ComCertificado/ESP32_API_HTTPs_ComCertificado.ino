...
void loop() {
  if ((millis() - lastTime) > timerDelay) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // Dolar:
      String bidStr1 = myObject["USDBRL"]["bid"];
      float bid1 = bidStr1.toFloat();
      Serial.print("Cotação do Dólar (USD): R$ ");
      Serial.println(bid1, 2);

      // Euro:
      String bidStr2 = myObject["EURBRL"]["bid"];
      float bid2 = bidStr2.toFloat();
      Serial.print("Cotação Euro (EUR): R$ ");
      Serial.println(bid2, 2);

      // BitCoin:
      String bidStr3 = myObject["BTCBRL"]["bid"];
      float bid3 = bidStr3.toFloat();
      Serial.print("Cotação Bitcoin (BTC): R$ ");
      Serial.println(bid3, 2);

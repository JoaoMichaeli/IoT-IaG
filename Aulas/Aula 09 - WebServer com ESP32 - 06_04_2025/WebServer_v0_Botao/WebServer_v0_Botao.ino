      ...
      if (client.available()) {             // Se há bytes para ler do cliente
        ...
        if (c == '\n') {                    // Se o byte é um caractere de nova linha
          ...
          if (currentLine.length() == 0) {
            ...
            // Mostrar estado atual e botôes ON/OFF para GPIO 2
            client.println("<p>GPIO 2 - LED " + output2State + "</p>");
            if (output2State=="off") {
              client.println("<p><a href=\"/2/on\"><button class=\"button\">LIGAR LED</button></a></p>");
            } else {
              client.println("<p><a href=\"/2/off\"><button class=\"button button2\">DESLIGAR LED</button></a></p>");
            }
